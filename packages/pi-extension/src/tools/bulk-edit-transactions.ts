...
const Params = Type.Object({
  ...
  field: Type.Union([Type.Literal("account"), Type.Literal("payee"), Type.Literal("status")], {
    description:
      "The field to change on each matching transaction: 'account' moves a posting to a different account; 'payee' renames the payee; 'status' sets the transaction status marker.",
  }),
  new_value: Type.String({
    description:
      "The replacement value: for field 'account' the new account (e.g. expenses:food:groceries); for field 'payee' the new payee name; for field 'status' one of 'cleared', 'pending', or 'unmarked'.",
  }),
...
export const bulkEditTransactionsTool: ToolDefinition<typeof Params, ModifyResult> = {
  ...
  description:
    "Run an hledger query and change one field on every matching transaction: move a posting to a different account, rename the payee, or set the status. Edits are surgical; the ledger is validated and the whole batch reverts on error.",
  promptSnippet: "Bulk-edit a field (account, payee, or status) on transactions matching an hledger query",
  promptGuidelines: [
    ...
    "field 'status' sets the header status marker on every match: new_value 'cleared' (*), 'pending' (!), or 'unmarked' (no marker). Useful for reconciliation, e.g. marking a month of one account cleared; posting-level markers are left untouched.",
    ...
    const detail =
      result.field === "account"
        ...
        : result.field === "payee"
          ...
          : `${result.transactions} transaction(s) marked ${params.new_value}`;
