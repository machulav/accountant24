...
// E — the Investments view over the real wiring: the preload allowlist,
// the ledger_investments IPC round trip, and the sidebar view switch. A
...
test("opens the Investments view from the sidebar and returns to the chat", async () => {
  ...
  await window.getByRole("button", { name: "Investments" }).click();
  await expect(window.getByRole("heading", { name: "Investments" })).toBeVisible();
  await expect(window.getByText("No investments yet")).toBeVisible();
  ...
  await expect(window.getByRole("heading", { name: "Investments" })).not.toBeVisible();
  await expect(window.getByLabel("Message input")).toBeVisible();
});
