 ...
 * Observes both the scroller (viewport width changes) and its child table
 * (content growth while a table streams in).
 */
function useOverflowX() {
  ...
  useEffect(() => {
    ...
    const check = () => setOverflows(scroller.scrollWidth > scroller.clientWidth + 1);
    check();
...
/**
 * Markdown table renderer. Renders the table compact inline; when it overflows
 * the narrow chat column, an expand button opens the same table in a wide
 * dialog so every column is visible without horizontal scrolling.
 */
export function MarkdownTable({ className, children, ...props }: ComponentPropsWithoutRef<"table">) {
  const { scrollRef, overflows } = useOverflowX();
  ...
  return (
    <div className="aui-md-table-wrapper group/md-table relative my-3 w-full">
      ...
      {/* Actions hug the table's top-right corner and fade in on hover (or
          ...
          as the table scrolls. Expand shows only when the table overflows. */}
