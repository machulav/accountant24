...
// Spec: the placeholder starts as the plain prompt, and every 5s fades out
...
describe("useRotatingPlaceholder()", () => {
  ...
  const renderRotating = () => renderHook(() => useRotatingPlaceholder());
  ...
  it("should show the plain prompt and not be swapping initially", () => {
    const { result } = renderRotating();
    expect(result.current.placeholder).toBe(PLAIN_PROMPT);
    expect(result.current.isSwapping).toBe(false);
  });

  it("should not start swapping at 4999ms (just before the rotation interval)", () => {
    const { result } = renderRotating();
  ...
  it("should start swapping when the 5s rotation interval elapses", () => {
    const { result } = renderRotating();
    act(() => vi.advanceTimersByTime(ROTATE_MS));
  ...
  it("should stop rotating after unmount", () => {
    const { result, unmount } = renderRotating();
    rotateOnce();
    expect(result.current.placeholder).toBe(MENTION_TIP);
