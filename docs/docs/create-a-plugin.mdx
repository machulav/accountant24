...
Put reference material in the skill's `references/` folder, and point the agent at a file in the step that needs it. The agent loads reference files through progressive disclosure. It reads them only when they are needed.
...
The plugin appears in the [marketplace](/docs/marketplace) at the next refresh, within about 30 minutes, and anyone can install it from **Settings → Plugins**.
...
1. When the user confirms the install, the app downloads the repository as an archive over HTTPS and unpacks it to a temporary folder.
...
The app shows these messages in the install dialog when an install fails, or on the plugin's row in **Settings → Plugins** when an installed plugin is broken.
