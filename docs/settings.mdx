...
Accountant24 stores its settings in `app-settings.json` in the [workspace](/workspace). The app updates the file when you change options in **Settings**. You can also edit it by hand. Restart the app afterwards to apply the changes.
...
Remembers which [built-in plugins](/plugins#built-in-plugins) the app has already installed. The app never installs a repository on this list again, so a built-in plugin you uninstalled stays uninstalled. The app manages this key.
