...
describe("addTransaction() file routing", () => {
  test("should write to ledger/YYYY/MM.txt based on date", async () => {
    writeFileSync(join(LEDGER, "main.txt"), "");
    await addTransaction(basicParams);
    expect(existsSync(join(LEDGER, "2026", "03.txt"))).toBe(true);
  ...
  test("should return the full file path in result", async () => {
    writeFileSync(join(LEDGER, "main.txt"), "");
    const result = await addTransaction(basicParams);
    expect(result.fullFilePath).toBe(join(LEDGER, "2026", "03.txt"));
  });
});

// ── main.txt management ─────────────────────────────────────────

describe("addTransaction() main.txt management", () => {
  test("should add include directive for new monthly file", async () => {
    writeFileSync(join(LEDGER, "main.txt"), "; Accountant24\n");
    await addTransaction(basicParams);
